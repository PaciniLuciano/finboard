...
async def calcular_momento(ticker: str, mercado: str = "BR") -> dict:
    try:
        ...
        if close.isna().all():
            return {"ticker": ticker, "score_momento": 5.0, "erro": "Close todo NaN — ativo sem dados de preco"}

        # Preenche eventuais NaN isolados com forward-fill antes dos cálculos
        close = close.ffill().dropna()

        if len(close) < 50:
            return {"ticker": ticker, "score_momento": 5.0, "erro": "Historico insuficiente apos limpeza"}
        ...
        retorno_6m  = float(retorno_6m_raw)  if (retorno_6m_raw  is not None and not pd.isna(retorno_6m_raw))  else None
